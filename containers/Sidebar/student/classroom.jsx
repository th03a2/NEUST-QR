...
class Sidebar extends Component {
  constructor(props) {
    ...
    this.state = { subjects: this.advisory ? this.advisory.subjects : [] };
  ...
  render() {
    ...
    const writer = this.state.subjects.map(subject => {
      return (
        <Menu.Item style={submenuStyle} key={subject.title}>
          <Link style={submenuColor} to={`${url}/cr/${subject.id}/subject`}>{subject.title}</Link>
        </Menu.Item>
      )
    });
    return (
